...
function DeliveredPackagesView({ packages, onOpenInvoice }: { packages: ZonePackage[]; onOpenInvoice: (invoiceId: string) => void }) {
  ...
  return <section className="rounded-2xl border border-emerald-200 bg-emerald-50/60 p-4">
    <div className="mb-3 flex items-center justify-between gap-3">
      <div><h3 className="text-base font-black text-[#0a2b61]">Colis remis</h3><p className="mt-1 text-xs text-slate-600">Historique des colis confirmés au point de retrait.</p></div>
      <span className="rounded-full bg-white px-2.5 py-1 text-xs font-bold text-emerald-700">{packages.length}</span>
    </div>
    <div className="space-y-2">{ordered.map((item) => <article key={item.id} className="rounded-xl border border-emerald-100 bg-white p-3">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div><p className="text-[11px] font-bold uppercase tracking-wide text-slate-400">Numéro de suivi</p><p className="break-all text-sm font-black text-[#0a2b61]">{packageReference(item)}</p><SpecialPackageNotice item={item} compact /></div>
        <StatusChip status={DONE} />
      </div>
      <div className="mt-3 grid gap-2 text-xs sm:grid-cols-2 lg:grid-cols-4">
        <p className="rounded-lg bg-slate-50 px-2.5 py-2"><span className="block text-slate-400">Client</span><b className="text-slate-700">{item.customer_code}{item.customer_name ? " · " + item.customer_name : ""}</b></p>
        <p className="rounded-lg bg-slate-50 px-2.5 py-2"><span className="block text-slate-400">Facture</span>{item.invoice_id ? <button type="button" onClick={() => onOpenInvoice(item.invoice_id)} className="font-bold text-indigo-700 hover:underline">{item.invoice_number || "Voir la facture"}</button> : <b className="text-slate-700">—</b>}</p>
        <p className="rounded-lg bg-slate-50 px-2.5 py-2"><span className="block text-slate-400">Date de remise</span><b className="text-slate-700">{item.delivered_at ? dateText(item.delivered_at) : "Non archivée"}</b></p>
        <p className="rounded-lg bg-slate-50 px-2.5 py-2"><span className="block text-slate-400">Quantité</span><b className="text-slate-700">{item.quantity} article{item.quantity > 1 ? "s" : ""}</b></p>
      </div>
    </article>)}</div>
  </section>;
}
...
function ClientDossiersView({ dossiers, focusedPackageId, expanded, onExpand, selectedPackageIds, confirmedParcelIds, releasing, onToggleSelection, onSelectCustomerPackages, onConfirmSelection, onStartPayment, onOpenInvoice }: {
  ...
  return <div className="grid gap-3 lg:grid-cols-2">{dossiers.map((dossier) => {
    ...
    const miami = sortPackagesAvailableFirst(counts.active.filter((item) => item.status === "Reçu à Miami"));
    const available = sortPackagesAvailableFirst(counts.active.filter(isReadyForPickup));
    const haiti = sortPackagesAvailableFirst(counts.active.filter((item) => item.status === "Arrivé en Haïti" || item.status === "En route vers agence"));
    const transit = sortPackagesAvailableFirst(counts.active.filter((item) => !isReadyForPickup(item) && item.status !== "Reçu à Miami" && item.status !== "Arrivé en Haïti" && item.status !== "En route vers agence"));
    ...
    return <article id={"dossier-" + dossier.customerCode} key={dossier.customerCode} className={cn("scroll-mt-6 overflow-hidden rounded-xl border bg-white sm:rounded-2xl", balanceUsd > 0.01 ? "border-amber-300" : "border-slate-200")}>
      <button type="button" onClick={() => onExpand(open ? null : dossier.customerCode)} className="flex w-full items-center justify-between gap-3 p-3 text-left hover:bg-slate-50 sm:p-4"><div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 sm:text-[11px]">Dossier client</p><h3 className="mt-0.5 truncate text-lg font-black text-[#0a2b61] sm:text-xl">{dossier.customerCode}{dossier.customerName && <span className="ml-1 text-xs font-semibold text-slate-500 sm:text-sm">· {dossier.customerName}</span>}</h3><div className="mt-2 flex flex-wrap gap-1.5 text-[11px] font-semibold sm:mt-3 sm:gap-2 sm:text-xs"><span className="rounded-lg bg-blue-50 px-2 py-1 text-blue-700">En route : {counts.enRoute}</span><span className="rounded-lg bg-orange-50 px-2 py-1 text-orange-700">Disponibles : {counts.available}</span></div></div><span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-[#edf3ff] text-[#0c397a] sm:h-10 sm:w-10"><ChevronDown size={18} className={open ? "rotate-180 transition-transform" : "transition-transform"} /></span></button>
      {open && <div className="space-y-3 border-t border-slate-100 bg-slate-50/70 p-2.5 sm:space-y-4 sm:p-3">
        ...
        <DossierSection title="Colis arrivés en Haïti / vers l’agence" subtitle="Étape suivante avant la disponibilité au point de retrait." packages={haiti} tone="blue" />
        <DossierSection title="Colis en transit ou en traitement" subtitle="Colis encore en acheminement ou en traitement." packages={transit} />
        <DossierSection title="Colis reçus à Miami" subtitle="Première étape : ces colis restent sous les colis plus avancés." packages={miami} />
...
function PackageCard({ item, ready, delivered, selected, releasing, confirmed, onToggleSelection, onStartPayment, onOpenInvoice }: { item: ZonePackage; ready: boolean; delivered: boolean; selected: boolean; releasing: boolean; confirmed: boolean; onToggleSelection: () => void; onStartPayment: () => void; onOpenInvoice: () => void }) {
  ...
  return <div className={cn("rounded-xl border bg-white p-2.5 sm:p-3", selected ? "border-emerald-400 ring-2 ring-emerald-100" : "border-slate-200")}>
    <div className="flex flex-wrap items-start justify-between gap-2"><div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-wide text-slate-400 sm:text-[11px]">Numéro de suivi</p><p className="break-all text-[13px] font-extrabold text-[#0a2b61] sm:text-sm">{packageReference(item)}</p></div><StatusChip status={item.status} /></div>
    {item.content && <p className="mt-1 text-xs text-slate-600 sm:mt-2 sm:text-sm"><span className="font-semibold text-slate-800">Marchandise :</span> {item.content}</p>}
    <SpecialPackageNotice item={item} />
    <div className="mt-1.5 flex flex-wrap gap-1.5 text-[11px] text-slate-500 sm:mt-2 sm:gap-2 sm:text-xs"><span className="rounded-lg bg-slate-100 px-2 py-1 font-semibold">Qté : {item.quantity}</span>{item.received_at && <span className="rounded-lg bg-blue-50 px-2 py-1 text-blue-700">Miami : {dateText(item.received_at)}</span>}{item.invoice_number && (item.invoice_id ? <button type="button" onClick={onOpenInvoice} className="rounded-lg bg-indigo-50 px-2 py-1 font-semibold text-indigo-700 transition hover:bg-indigo-100 hover:text-indigo-900" title="Voir le détail de cette facture">{item.invoice_number} · {item.invoice_payment_status}</button> : <span className="rounded-lg bg-indigo-50 px-2 py-1 font-semibold text-indigo-700">{item.invoice_number} · {item.invoice_payment_status}</span>)}</div>
    {item.invoice_payment_details && <p className="mt-1.5 text-[11px] font-bold text-emerald-700 sm:mt-2 sm:text-xs">{item.invoice_payment_details}</p>}
    {ready && !hasBalance && !canRelease && item.invoice_id && <button type="button" onClick={onStartPayment} className="mt-2 flex min-h-10 w-full items-center justify-center gap-2 rounded-xl border border-orange-200 bg-orange-50 px-3 text-sm font-bold text-[#bd450b] hover:bg-orange-100 sm:mt-3 sm:min-h-11"><Banknote size={17} />Enregistrer un paiement</button>}
    {ready && hasBalance && <p className="mt-2 rounded-lg bg-amber-50 px-3 py-2 text-xs font-bold text-amber-800 sm:mt-3">Le solde client doit être réglé avant la remise.</p>}
    {ready && !confirmed && <button type="button" disabled={!canRelease || releasing} onClick={onToggleSelection} className={cn("mt-2 flex min-h-10 w-full items-center justify-center gap-2 rounded-xl border px-3 text-sm font-bold disabled:cursor-not-allowed sm:mt-3 sm:min-h-11", selected ? "border-emerald-600 bg-emerald-600 text-white" : "border-[#0b3270] bg-white text-[#0b3270] hover:bg-[#edf3ff]", (!canRelease || releasing) && "border-slate-200 bg-slate-100 text-slate-400")}><CheckCircle2 size={17} />{selected ? "Sélectionné pour la remise" : canRelease ? "Sélectionner pour la remise" : item.invoice_id ? "Paiement complet requis" : "Facture requise"}</button>}
    {confirmed && <div className="mt-2 flex min-h-10 w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 px-3 text-sm font-bold text-white sm:mt-3 sm:min-h-11"><CheckCircle2 size={17} />Livré · remise confirmée</div>}
    {delivered && <p className="mt-2 flex items-center gap-2 text-sm font-bold text-emerald-700 sm:mt-3"><CheckCircle2 size={16} />Remis au client</p>}
  </div>;
}
...
function StatusChip({ status }: { status: string }) { const tone = status === DONE ? "bg-emerald-100 text-emerald-700" : READY_STATUSES.has(status) ? "bg-orange-100 text-orange-700" : "bg-blue-100 text-blue-700"; return <span className={cn("rounded-lg px-2 py-1 text-[11px] font-bold sm:px-2.5 sm:text-xs", tone)}>{status}</span>; }
