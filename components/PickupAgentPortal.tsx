...
function ParcelRow({ item }: { item: ZonePackage }) {
  return <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg bg-white px-3 py-2"><div className="min-w-0"><p className="break-all text-sm font-bold text-[#0a2b61]">{packageReference(item)}</p><p className="mt-0.5 text-xs text-slate-500">Quantité : {item.quantity}{item.content ? " · " + item.content : ""}</p><SpecialPackageNotice item={item} compact /></div><StatusChip status={item.status} /></div>;
}

function DossierInvoices({ invoices, onOpenInvoice }: { invoices: ZoneInvoice[]; onOpenInvoice: (invoiceId: string) => void }) {
  return <section className="rounded-xl border border-indigo-100 bg-indigo-50/50 p-3"><div className="mb-2 flex items-start justify-between gap-3"><div><h4 className="text-sm font-black text-[#0a2b61]">Factures du client</h4><p className="mt-0.5 text-xs text-slate-500">Cliquez sur un numéro pour voir tous les colis de la facture, payés ou livrés.</p></div><span className="rounded-full bg-white px-2 py-1 text-xs font-bold text-slate-600">{invoices.length}</span></div>{invoices.length ? <div className="space-y-2">{invoices.map((invoice) => <button key={invoice.id} type="button" onClick={() => onOpenInvoice(invoice.id)} className="flex w-full flex-wrap items-center justify-between gap-3 rounded-lg border border-indigo-100 bg-white px-3 py-2 text-left transition hover:border-indigo-300 hover:bg-indigo-50/40"><span className="min-w-0"><span className="block text-sm font-bold text-indigo-700 hover:underline">{invoice.invoice_number}</span><span className="mt-0.5 block text-xs text-slate-500">{invoice.package_count} colis · {dateText(invoice.created_at)}</span></span><span className="flex flex-wrap items-center justify-end gap-2"><DeliveryChip status={invoice.delivery_status} /><PaymentChip status={invoice.payment_status} /></span></button>)}</div> : <p className="rounded-lg bg-white/80 px-3 py-3 text-sm text-slate-500">Aucune facture finalisée pour ce client.</p>}</section>;
}
