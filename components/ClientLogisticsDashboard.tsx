...
import {
  BarChart3, Check, ChevronLeft, ChevronRight, Clipboard, Handshake, MapPin, MessageCircle,
...
function QuickAction({
  ...
  return <button type="button" onClick={onClick} className="group flex min-h-[108px] flex-col items-center justify-center gap-1.5 rounded-3xl bg-white px-2 text-center shadow-[0_12px_30px_-24px_rgba(12,67,143,.55)] ring-1 ring-sky-100 transition hover:-translate-y-0.5 hover:border-sky-200 hover:shadow-md">
    <span className="relative">
      <span className="grid h-12 w-12 place-items-center rounded-2xl border border-sky-100 bg-[#eaf4ff] text-[#145ca8] shadow-sm transition group-hover:scale-105 group-hover:bg-[#dcebff]">
        <Icon size={22} strokeWidth={2.1} />
      </span>
      {typeof count === "number" && count > 0 && (
        <span className="absolute -right-1.5 -top-1.5 grid h-5 min-w-5 place-items-center rounded-full bg-[#145ca8] px-1 text-[10px] font-black text-white ring-2 ring-white">
          {count > 99 ? "99+" : count}
        </span>
      )}
    </span>
...
function HomeParcelIllustration() {
  return <div aria-hidden className="relative h-[126px] w-[132px] shrink-0 overflow-visible">
...
function JourneyLine() {
  ...
  return <div className="mt-5 flex items-start">
    {steps.map((step, index) => <div key={step} className="flex min-w-0 flex-1 items-start last:flex-none">
      <div className="flex min-w-0 flex-col items-center"><span className="grid h-6 w-6 place-items-center rounded-full border-2 border-[#145ca8] bg-white text-[10px] font-black text-[#145ca8]">{index + 1}</span><span className="mt-1.5 max-w-[60px] text-center text-[9px] font-bold leading-tight text-slate-600">{step}</span></div>
      {index < steps.length - 1 && <span className="mt-3 h-px min-w-2 flex-1 bg-[#9ec5ed]" />}
    </div>)}
  </div>;
}
...
export function ClientLogisticsDashboard({ clientName, destination, balanceUsd, estimatedUsd, availableCount, receptionCount, retraitCount, deliveredCount, unreadNotifications, onNavigate }: DashboardProps) {
  return <section className="space-y-5">
    ...
    <section><div className="mb-3 flex items-center justify-between"><h2 className="text-lg font-black tracking-tight text-slate-900">Vos colis</h2><span className="text-[11px] font-bold text-slate-400">Vue d&apos;ensemble</span></div><div className="grid grid-cols-4 gap-2.5">
