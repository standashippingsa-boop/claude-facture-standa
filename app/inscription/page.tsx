...
const schema = z.object({
  ...
  city: z.string().min(1, "Vil obligatwa"),
  ...
  confirm: z.string()
}).refine((d) => d.password === d.confirm, { path: ["confirm"], message: "De modpas yo pa menm" });
...
export default function InscriptionPage() {
  ...
  const onSubmit = async (f: Form) => {
    ...
    try {
      ...
      if (!uid) throw new Error("Kreyasyon kont lan echwe — eseye ankò.");
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="card p-6 md:p-8 space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <F name="city" label="Vil *" placeholder="Ex: Port-au-Prince" />
            <F name="city2" label="2yèm vil (opsyonèl)" placeholder="Si ou pran koli nan 2 vil" />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <F name="password" label="Modpas *" type="password" />
            <F name="confirm" label="Konfimasyon modpas *" type="password" />
          </div>
          ...
          <button type="submit" className="btn w-full justify-center py-3" disabled={isSubmitting}>
            {isSubmitting ? "Ap kreye kont lan..." : "Kreye kont mwen"}
          </button>
