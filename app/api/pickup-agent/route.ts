  ...
  try {
    ...
    if (body?.action === "release_many") {
      ...
      if (pushConfig) {
        ...
        // touye lè fonksyon an reponn, epi push la pa janm pati. De kanal
        // (Web Push + FCM app Android) an paralèl, youn pa dwe bloke lòt la.
        const copy = { title: "Colis remis", text: packageIds.length > 1 ? `${packageIds.length} colis vous ont été remis.` : "Votre colis vous a été remis." };
        await Promise.allSettled([
          sendPushToCustomer(pushConfig, customerCode, copy),
    ...
    if (body?.action === "release") {
      ...
      const pushConfig = getSupabaseAdminConfig();
      if (pushConfig) {
        const copy = { title: "Colis remis", text: "Votre colis vous a été remis." };
        await Promise.allSettled([
          sendPushToCustomer(pushConfig, parcel.customer_code, copy),
          sendFcmToCustomer(pushConfig, parcel.customer_code, copy)
        ]);
      }
