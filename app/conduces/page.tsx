...
export default function ConducesPage() {
  ...
  /** Katab yo LOUVRI pa defo — conduces yo dwe vizib san peze anyen. */
    ...
    <div className="space-y-4 pb-28">
      ...
      <div className="flex gap-1 border-b border-line overflow-x-auto">
        {([["toutes", "Toutes", all.length], ["actives", "En cours", actives.length], ["historique", "Historique", historique.length]] as const).map(
          ([k, label, n]) => (
            <button key={k} onClick={() => { setTab(k); setSel(new Set()); setClosedDays(new Set()); }}
        ...
        <div className="space-y-2.5">
          {classeurs.map((cl) => (
            <ClasseurCard key={cl.key} cl={cl}
              open={!closedDays.has(cl.key)}
              onToggle={() => setClosedDays((p) => {
                const n = new Set(p); if (n.has(cl.key)) n.delete(cl.key); else n.add(cl.key); return n;
              })}
